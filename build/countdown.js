class CountdownTimer {
    ...
    countDown() {
        ...
        const sec = (Math.floor(((this.tl - Number(today)) % (24 * 60 * 60 * 1000)) / 1000) %
            60) %
            60;
        if (this.tl - Number(today) > 0 || this.tl - Number(today) < -86400000) {
            const timer = `<p>やまのくの誕生日まで</p><div class="number"><p><span class="number-day">${day}</span><span class="number_value">day</span></p><p><span class="number-hour">${this.addZero(hour)}</span><span class="number_value">hour</span></p><p><span class="number-min">${this.addZero(min)}</span><span class="number_value">min</span></p><p><span class="number-sec">${this.addZero(sec)}</span><span class="number_value">sec</span></p></div>`;
...
window.onload = () => {
    const now = new Date();
    let tl = Number(new Date(now.getFullYear(), 9, 30)); // eslint-disable-line
